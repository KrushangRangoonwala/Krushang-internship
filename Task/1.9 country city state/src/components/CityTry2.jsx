...
const CityTry2 = () => {
    ...
    const [city, setCity] = useState([])  // unexpacted error : solution : should not define any variable with `state` instead use this `city`  
    ...
    function selectCountry(i) {
        ...
        setState_name(shownState[i].name);
        stateByCountry(shownState[i].id);
    }
    ...
    async function getAllCountryState() {
        ...
        (statesNames && statesNames.length > 0) && setCity(statesNames);
    }
    ...
    useEffect(() => {
        ...
        temp = totalState.filter((val) => searchPettern.test(val.name));
        if (temp.length > 0 || search.length > 0) {
            setShownState(shownState => temp);
        }
    }, [search]);

    async function addState(country_id, state_name2) {
        let val = {
                ...
                <form onSubmit={formik.handleSubmit}>
                    ...
                    >
                        <option value='' onClick={async () => {  //async await neccessary
                            ...
                            await setState_name('');      // must execute next , if u run this first then `{(state_name.length > 0) ?` this line run 1st and in 'setCity' we have all previous country's state
                        }}>Select Country</option>

                        {shownState.map((val, index) => (
